...
class Edrone():
    ...
    def __init__(self):
        ...
        self.setpoints =[]
        self.target_setpoints=[[4,5,5,5]] # Setpoints after which we need to search for marker
    ...
    def search_marker(self):
        ...
        if(self.detected==0 and self.search_change==1): # If drone is not detecting and drone is not landing,this happens when it is either ascendind to detect or descending
            ...
            if(self.reached==1 and self.wait==0):  # If threshold box is reached then give coordinates to descend the drone,self.wait is added so that only one coordinate is given
                if(self.i==1):
                    alti=self.setpoints[self.search_index][2]-16.75+0.33
                else:
                    alti=self.setpoints[self.search_index][2]-16.75
        ...
        if(elapsed_time>=0.001 and len(self.search_setpoints)==1): # Sample time
            ...
            if(lat+longt+alt==3): # If drone is landing then program does not go inside it as no coordinate is given then,only throttle values are given
                ...
                if(self.reached==1 and self.wait==1): # If drone has descended after detecting
                    ...
                    self.setpoints.insert(self.r,[self.current_coord[0],self.current_coord[1],self.current_coord[2]+2,0])
...
#################################################### Main function #################################################################################
def get_dist(coord):
    x=(abs(coord[0]-18.9998102845))*110692.0702932625 # Calculating total latitude distance between current setpoint and destination setpoint in metres
    y=(abs(coord[1]-72.000142461))*105292.0089353767 # Calculating total longitude distance between current setpoint and destination setpoint in metres
                        
    path_distance=(x**2+y**2)**0.5 # Calculating distance between current setpoint and destination setpoint in metres  
    return path_distance

if __name__ == '__main__':
    delivery_lst1=["A1","A2","A3","B1","B2","B3","C1","C2","C3"]
    return_lst=["X1","X2","X3","Y1","Y2","Y3","Z1","Z2","Z3"]
    # delivery_lst=["B2","A2","C1","B1","C3","A1","C2","B3","A3"]
    ...
    return_grid=[[18.9999367615,72.000142461,16.757981,"X1"],[18.9999367615,72.000156706,16.757981,"X2"],[18.9999367615,72.000170951,16.757981,"X3"],[18.9999503135,72.000142461,16.757981,"Y1"],[18.9999503135,72.000156706,16.757981,"Y2"],[18.9999503135,72.000170951,16.757981,"Y3"],[18.9999638655,72.000142461,16.757981,"Z1"],[18.9999638655,72.000156706,16.757981,"Z2"],[18.9999638655,72.000170951,16.757981,"Z3"]]
    csv_list_delivery=[] # To store data from csv file
    csv_list_return=[] # To store data from csv file
    with open("/home/pranav/catkin_ws/src/vitarana_drone/scripts/manifest.csv","r") as csv_file:
    ...
    #print(len(csv_list_delivery),len(csv_list_return))

    for i in range(0,len(csv_list_delivery)):
        coords=csv_list_delivery[i][2].split(",")
        coord=[float(coords[0]),float(coords[1]),float(coords[2])]
        csv_list_delivery[i].append(coord)
        csv_list_delivery[i].append(get_dist(coord))

        coords1=csv_list_return[i][1].split(",")
        coord1=[float(coords1[0]),float(coords1[1]),float(coords1[2])]
        csv_list_return[i].append(coord1)
    
    
    #to sort according to minimum distance
    for i in range(0,len(csv_list_delivery)-1):                   
        for j in range(0,len(csv_list_delivery)-i-1):
            if(csv_list_delivery[j][4]>csv_list_delivery[j+1][4]):
                csv_list_delivery[j],csv_list_delivery[j+1]=csv_list_delivery[j+1],csv_list_delivery[j]
                csv_list_return[j],csv_list_return[j+1]=csv_list_return[j+1],csv_list_return[j]
    ...
    #             e_drone.setpoints.append([delivery_grid[index_j][0],delivery_grid[index_j][1],delivery_grid[index_j][2]+3,0,"NULL"])
    #             e_drone.setpoints.append([delivery_grid[index_j][0],delivery_grid[index_j][1],delivery_grid[index_j][2],0,delivery_lst[index_i]])
    
        #for index_k in range(0,len(csv_list_delivery)):
    
    #to enter setpoints
    for i in range(0,len(csv_list_delivery)):
        ind=delivery_lst1.index(csv_list_delivery[i][1])
        coord=[delivery_grid[ind][0],delivery_grid[ind][1],delivery_grid[ind][2]]
        e_drone.setpoints.append([coord[0],coord[1],coord[2]+3,0])
        e_drone.setpoints.append([coord[0],coord[1],coord[2],0])

        coord2=csv_list_delivery[i][3]
        e_drone.setpoints.append([coord2[0],coord2[1],coord2[2]+16.75,0])

        coord_pick=csv_list_return[i][3]
        e_drone.setpoints.append([coord_pick[0],coord_pick[1],coord_pick[2]+8,0])
        e_drone.setpoints.append([coord_pick[0],coord_pick[1],coord_pick[2],0])
        e_drone.setpoints.append([coord_pick[0],coord_pick[1],coord_pick[2]+8,0])

        ind_return=return_lst.index(csv_list_return[i][2])
        coord_r=[return_grid[ind_return][0],return_grid[ind_return][1],return_grid[ind_return][2]]
        e_drone.setpoints.append([coord_r[0],coord_r[1],coord_r[2]+8,0])
        e_drone.setpoints.append([coord_r[0],coord_r[1],coord_r[2],0])


    
    print(e_drone.setpoints)     
    ...
    rospy.loginfo(csv_list_delivery)
    rospy.loginfo(csv_list_return)
